...
// TFTを行えるようにします
//**************************************************
mrb_value Is_useTFT(mrb_state *mrb, mrb_value self, int mode)
{
int ret = 0;

#if BOARD == BOARD_GR || FIRMWARE == SDBT || FIRMWARE == SDWF || BOARD == BOARD_P05 || BOARD == BOARD_P06
	ret = TFT_Init(mrb);		//TFT関連メソッドの設定
#endif
...
// 0:使用不可, 1:使用可能
//**************************************************
mrb_value mrb_system_useTFT(mrb_state *mrb, mrb_value self)
{
	return Is_useTFT(mrb, self, 0);
}
//**************************************************
...
int ret = 0;
	...
	else if(strcmp(strName, TFT_CLASS) == 0){
		if(mode == 0){
			return Is_useTFT(mrb, self, 0);
		}
		else{
...
void sys_Init(mrb_state *mrb)
{
	...
	mrb_define_module_function(mrb, systemModule, "useTFT", mrb_system_useTFT, MRB_ARGS_NONE());
